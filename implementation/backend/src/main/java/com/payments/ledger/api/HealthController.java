package com.payments.ledger.api;

import com.payments.ledger.api.dto.HealthResponse;
import com.payments.ledger.repository.LedgerRepository;
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RestController;

@RestController
public class HealthController {

  private final LedgerRepository ledgerRepository;

  public HealthController(LedgerRepository ledgerRepository) {
    this.ledgerRepository = ledgerRepository;
  }

  @GetMapping("/health")
  public HealthResponse health() {
    return new HealthResponse("ok");
  }

  /** Readiness probe (SPEC §5 OP-006). */
  @GetMapping("/ready")
  public ResponseEntity<HealthResponse> ready() {
    if (ledgerRepository.isHealthy()) {
      return ResponseEntity.ok(new HealthResponse("ok"));
    }
    return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
        .body(new HealthResponse("database unavailable"));
  }
}
